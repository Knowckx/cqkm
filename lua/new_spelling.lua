...
local function spell_phrase(s, spll_rvdb)
	...
	elseif #chars == 3 then
		return subspelling(spellings[1], 1, 0) ..
					 ...
					 subspelling(spellings[3] .. sup, 1, 0)
	else
		return subspelling(spellings[1], 1, 0) ..
					 subspelling(spellings[2], 1, 0) ..
					 subspelling(spellings[3], 1, 0) ..
					 subspelling(spellings[4], 1, 0)
	end
end
		...
		for cand in input:iter() do
			if isgb2312(cand,env)==1 and env.engine.context:get_option("GB2312") or not env.engine.context:get_option("GB2312") then
				...
				if cand.type == 'simplifier' and env.name_space == 'new_for_rvlk' then
					...
					if script_text:find("^i[a-z]*") and not script_text:find("%p$") or script_text:find("^([%/])[a-z]*") and not script_text:find("%p$") then
